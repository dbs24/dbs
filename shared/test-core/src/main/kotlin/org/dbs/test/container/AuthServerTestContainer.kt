...
class AuthServerTestContainer : AbstractTestContainer2() {

    @Suppress(UNCHECKED_CAST)
    override fun <T : GenericContainer<*>> createContainer(): T =
        getProperty(USER_DIR).also { logger.debug { "user.dir = '$it'" } }
            .let {
                val workDir = "/home/user/Projects/uonmap/sss_services/auth-server"
                val filePath = "$workDir/Dockerfile"
                val path = Path(filePath)
                //val regExp = "(SUCCESSFULLY|STARTED)"
                val port = 1443

                //val path: URI = Demo::class.java.getResource("/PostgreSQLde").toURI()
                val image = ImageFromDockerfile().withDockerfile(path)

                val imageName = DockerImageName.parse(image.get())
                val times = 1
                //val wait = WaitingConsumer()
                //val toString = ToStringConsumer()
                val network = Network.newNetwork()
                //.asCompatibleSubstituteFor(PostgreSQLContainer.IMAGE)

                GenericContainer(imageName)
                    .withNetwork(network)
                    .withEnv(SERVER_SSL_ENABLED, SERVER_SSL_DISABLED)
                    .withExposedPorts(port)
                    //.withExtraHost("test.k11dev.tech", "192.168.1.1")
                    .withAccessToHost(true)
                    .withNetworkAliases("test.k11dev.tech")
                    //.withLogConsumer(Slf4jLogConsumer(logger.delegate as Logger))
                    //.withLogConsumer(wait.andThen(toString))
                    .withStartupAttempts(times)
//                    .setWorkingDirectory(workDir)
                    .waitingFor(
                        Wait.forHttp("/actuator/health")
                            //"/api/smart-safe-school/liveness")
                            //.usingTls()
                            .forStatusCode(OK.value())
                            .withStartupTimeout(TIMEOUT_60_SEC)
                    )
//                    .waitingFor(Wait.forLogMessage(regExp, times))
//                    .withStartupCheckStrategy(OneShotStartupCheckStrategy().withTimeout(TIMEOUT_60_SEC))
//                    .withStartupCheckStrategy(MinimumDurationRunningStartupCheckStrategy(TEN_SECONDS))
//                    .waitingFor(LogMessageWaitStrategy().withRegEx(regExp).withTimes(times).withStartupTimeout(TIMEOUT_60_SEC))
                        as T
            }
