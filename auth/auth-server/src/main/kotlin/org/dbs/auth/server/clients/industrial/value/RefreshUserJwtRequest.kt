package org.dbs.auth.server.clients.industrial.value
...
import org.dbs.auth.server.clients.industrial.grpc.convert.UserRefreshJwtConverter as CONV
import org.dbs.rest.dto.value.RefreshJwtRequest as REQ
import org.dbs.rest.dto.value.RefreshJwtResponse as RESP

@JvmInline
value class RefreshUserJwtRequest<R : ServerRequest>(private val serverRequest: R) :
    GenericRequest<R>,
    ...
    override suspend fun buildResponse(processor: H1_PROCESSOR<R>) = processor.createResponse(
        ...
        serverRequest.run {
            restFulService.buildMonoResponse(this, REQ::class)
            {
                ...
                { with(authServerGrpcService) { userRefreshJwtInternal(it, ip(), userAgent()) } }
            }
        }
    }
}
