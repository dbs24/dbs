...
import org.dbs.auth.server.clients.industrial.grpc.convert.UserLoginConverter as H1_CONV
...
@JvmInline
value class LoginUserRequest<R : ServerRequest>(private val serverRequest: R) :
    ...
    override suspend fun buildResponse(
        ...
        serverRequest.run {
            restFulService.buildMonoResponse(this, H1_REQ::class)
            {
                ...
                { with(authServerGrpcService) { loginUserInternal(it, ip(), userAgent()) } }
            }
        }
    }
}
